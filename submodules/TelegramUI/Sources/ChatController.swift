...
public final class ChatControllerImpl: TelegramBaseController, ChatController, GalleryHiddenMediaTarget, UIDropInteractionDelegate {
    ...
    public init(
        ...
        self.openedFromChatList = openedFromChatList
        ...
        let controllerInteraction = ChatControllerInteraction(openMessage: { [weak self] message, params in
        ...
        }, editMessage: { [weak self] message in
            guard let self else { return false }
            
            let canEdit = canEditMessage(
                context: self.context,
                limitsConfiguration: context.currentLimitsConfiguration.with { EngineConfiguration.Limits($0) },
                message: message
            )
            guard canEdit else { return false }
            interfaceInteraction?.setupEditMessage(message.id, { _ in })
            return true
        }, forwardMessage: { [weak self] message, completion in
            guard let self else { return }
            _ = (self.context.sharedContext.chatAvailableMessageActions(
                engine: self.context.engine,
                accountPeerId: self.context.account.peerId,
                messageIds: [message.id],
                keepUpdated: false
            ) |> deliverOnMainQueue)
            .start(next: { [weak self] actions in
                guard let self else { return }
                let isCopyProtected = message.isCopyProtected() || actions.isCopyProtected
                if actions.options.contains(.forward) && !isCopyProtected {
                    self.forwardMessages(messageIds: [message.id])
                    completion(true)
                } else {
                    completion(false)
                }
            })
        }, forwardMessageToSaved: { [weak self] message, completion in
            guard let self else { return }
            _ = (self.context.sharedContext.chatAvailableMessageActions(
                engine: self.context.engine,
                accountPeerId: self.context.account.peerId,
            ...
            |> deliverOnMainQueue).startStandalone(next: { [weak self] peer, message in
                ...
                if case let .customChatContents(customChatContents) = strongSelf.presentationInterfaceState.subject {
                    switch customChatContents.kind {
                    case .wall:
                        items.append(.action(ContextMenuActionItem(text: "Wall.Conversation.ContextMenuLeaveChannel".tp_loc(lang: strongSelf.presentationData.strings.baseLanguageCode), icon: { theme in
                                return generateTintedImage(image: TPIconManager.shared.icon(.contextMenuLeave), color: theme.actionSheet.primaryTextColor)
                            }, action: { _, f in
                                f(.dismissWithoutContent)
                                ...
                                let text = "Wall.LeaveChannel.Text".tp_loc(lang: strongSelf.presentationData.strings.baseLanguageCode, with:  peerName)
                                
                                let controller = textAlertController(
                                    context: strongSelf.context,
                                    title: "Wall.LeaveChannel.Title".tp_loc(lang: strongSelf.presentationData.strings.baseLanguageCode),
                                    text: text,
                                    actions: [
                                        TextAlertAction(
                                            type: .destructiveAction,
                                            title: "Wall.LeaveChannel.Confirm".tp_loc(lang: strongSelf.presentationData.strings.baseLanguageCode),
                                            action: { [weak self] in
                                                ...
                                                Queue.mainQueue().after(0.1) {
                                                    let signal = self.context.engine.peers.removePeerChat(peerId: peer.id, reportChatSpam: false, deleteGloballyIfPossible: false)
                                                    cancelImpl = { [weak disposable] in
                                                        disposable?.set(nil)
                                                    }
                                                    
                                                    disposable.set((signal
                                                        |> deliverOnMainQueue).startStrict(completed: { [weak controller] in
                                        ...
                                        TextAlertAction(
                                            type: .defaultAction,
                                            title: "Wall.LeaveChannel.Cancel".tp_loc(lang: strongSelf.presentationData.strings.baseLanguageCode),
                                            action: {
        ...
        self.chatTitleView?.longPressed = { [weak self] in
            if let strongSelf = self, let peerView = strongSelf.peerView, let peer = peerView.peers[peerView.peerId], peer.restrictionText(platform: "ios", contentSettings: strongSelf.context.currentContentSettings.with { $0 }) == nil && !strongSelf.presentationInterfaceState.isNotAccessible {
                if strongSelf.chatDisplayNode.contentHiddenOverlayNode.isVisible() {
                } else  if case .standard(.previewing) = strongSelf.mode {
                } else {
                    ...
                    if let strongSelf = self {
                        ...
                        if case let .messageOptions(_, _, info) = presentationInterfaceState.subject {
                            ...
                            if case .pinnedMessages = presentationInterfaceState.subject {
                                ...
                                if isPeerAllowed {
                                    strongSelf.chatTitleView?.hideAllIcons(!isPeerAllowed)
                                }
                                (strongSelf.chatInfoNavigationButton?.buttonItem.customDisplayNode as? ChatAvatarNavigationNode)?.setPeer(context: strongSelf.context, theme: strongSelf.presentationData.theme, peer: EnginePeer(peer), overrideImage: imageOverride, clipStyle: strongSelf.presentationData.theme.squareStyle ? .rect : .round, blurred: !isPeerAllowed)
                                ...
                                if case .standard(.previewing) = strongSelf.mode {
                                    ...
                                    (strongSelf.chatInfoNavigationButton?.buttonItem.customDisplayNode as? ChatAvatarNavigationNode)?.contextActionIsEnabled = isPeerAllowed && peer.restrictionText(platform: "ios", contentSettings: strongSelf.context.currentContentSettings.with { $0 }) == nil
                                }
                                ...
                                if let avatarNode = strongSelf.avatarNode, isPeerAllowed {
                ...
                |> deliverOnMainQueue).startStrict(next: { [weak self] peerView, messageAndTopic, savedMessagesPeer, onlineMemberCount, hasScheduledMessages, hasSearchTags, hasSavedChats, isPremiumRequiredForMessaging, managingBot, globalPrivacySettings in
                    if let strongSelf = self {
                        ...
                        if let savedMessagesPeerId {
                            ...
                            _ = ((context.childModeManager?.isPeerAllowed(peerId) ?? .single(true))
                                 |> take(1)
                                 |> deliverOnMainQueue
                            ).start(next: { [weak self] allowed in
                                (self?.chatInfoNavigationButton?.buttonItem.customDisplayNode as? ChatAvatarNavigationNode)?.setPeer(context: strongSelf.context, theme: strongSelf.presentationData.theme, peer: savedMessagesPeer?.peer, overrideImage: imageOverride, blurred: !allowed)
                            })
...
//            }
    ...
    func openStories(peerId: EnginePeer.Id, avatarHeaderNode: ChatMessageAvatarHeaderNodeImpl?, avatarNode: AvatarNode?) {
        if self.chatDisplayNode.contentHiddenOverlayNode.isVisible() {
            return
        }
