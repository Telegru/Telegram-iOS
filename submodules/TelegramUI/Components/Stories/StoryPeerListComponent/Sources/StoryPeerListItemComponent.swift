...
private func addSegmentedRoundedSquarePath(
    ...
    func getPathSegments(fromLength start: CGFloat, toLength end: CGFloat) -> [PathSegment] {
        ...
        let rotationLineLength = cornerRadius * sin(rotationFraction * .pi * 2)
        ...
        while remainingLength > 0 {
            ...
            if currentLength < halfSide && !completedStages[0] {
                let startX = x + currentLength
                let length = min(halfSide - currentLength, remainingLength)
            ...
            if currentLength < cornerPartsLength / 4 && !completedStages[1] {
                ...
                if arcLength == 0 {
                    ...
                    if arcLength <= 0 {
                        arcLength = max(remainingLength, currentLength)
                    }
                }
            ...
            if currentLength < sideLength && !completedStages[2] {
                let startY = y - halfSide + currentLength
            ...
            if currentLength < sideLength && !completedStages[4] {
                let startX = x + halfSide - currentLength
                let length = min(sideLength - currentLength, remainingLength)
                segments.append(.line(
                    from: CGPoint(x: startX, y: y + halfSize),
                    to: CGPoint(x: startX - length, y: y + halfSize)
            ...
            if currentLength < halfSide + rotationLineLength {
                let startX = x - sideLength / 2 + currentLength
                let length = min(sideLength + rotationLineLength - currentLength, remainingLength)
                segments.append(.line(
                    from: CGPoint(x: startX, y: y - halfSize),
                    to: CGPoint(x: startX + length, y: y - halfSize)
                ))
    ...
    for segmentIndex in startSegmentIndex...lastSegmentIndex {
        let segmentStartLength = CGFloat(segmentIndex) * (segmentLength + segmentSpacing) + segmentSpacing * 0.5 + cornerRadius * sin(rotationFraction * .pi * 2)
