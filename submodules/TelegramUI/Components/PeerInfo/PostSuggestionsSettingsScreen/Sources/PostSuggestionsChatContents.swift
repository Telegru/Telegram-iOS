...
public final class PostSuggestionsChatContents: ChatCustomContentsProtocol {
    ...
    public var disableFloatingDateHeaders: Bool = false
    
    public var isLoadingSignal: Signal<Bool, NoError> {
        .complete()
    }
    
    public func loadAll() {}
    
    public func loadMoreAt(messageIndex: MessageIndex, direction: ChatHistoryListLoadDirection) {}
    
    public func applyMaxReadIndex(for location: ChatLocation, contextHolder: Atomic<(any ChatLocationContextHolder)?>, messageIndex: MessageIndex) {}
}
