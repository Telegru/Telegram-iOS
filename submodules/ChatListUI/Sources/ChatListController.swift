...
public class ChatListControllerImpl: TelegramBaseController, ChatListController {
    ...
    private var isChildModeActive: Bool = false
    ...
    private let showRecentChatsValue = ValuePromise<Bool>(false)
    ...
    public init(context: AccountContext, location: ChatListControllerLocation, controlsHistoryPreload: Bool, hideNetworkActivityStatus: Bool = false, previewing: Bool = false, enableDebugActions: Bool) {
        ...
        self.recentChatsContainerNode = ChatListRecentChatsPaneNode(
            context: context,
        ...
        let primaryContext = ChatListLocationContext(
            ...
            showRecentChats: self.showRecentChatsValue.get()
        )
    ...
    private func updateThemeAndStrings() {
        ...
        if self.isNodeLoaded {
            ...
            self.recentChatsContainerNode.updatePresentationData(self.presentationData)
        }
    ...
    override public func loadDisplayNode() {
        ...
        self.chatListDisplayNode.mainContainerNode.peerSelected = { [weak self] peer, threadId, animated, activateInput, promoInfo in
            ...
            |> deliverOnMainQueue).start(next: { [weak self] combinedView in
                ...
                if case let .channel(channel) = peer, channel.flags.contains(.isForum), let threadId {
                    ...
                    self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: self.context, chatLocation: chatLocation, activateInput: (activateInput && !peer.isDeleted) ? .text : nil, openedFromChatList: true, scrollToEndIfExists: scrollToEndIfExists, animated: !scrollToEndIfExists, options: navigationAnimationOptions, parentGroupId: groupId._asGroup(), chatListFilter: self.chatListDisplayNode.mainContainerNode.currentItemNode.chatListFilter?.id, completion: { [weak self] controller in
                        guard let self else {
                            return
                        }
                    ...
                    if let strongSelf = strongSelf {
                        ...
                        if let navigationController = strongSelf.navigationController as? NavigationController {
                            ...
                            if case let .channel(channel) = peer, channel.flags.contains(.isForum), let threadId {
                                ...
                                strongSelf.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: strongSelf.context, chatLocation: .peer(peer), openedFromChatList: dismissSearch, purposefulAction: { [weak self] in
                                    self?.deactivateSearch(animated: false)
                                }, scrollToEndIfExists: scrollToEndIfExists, options: navigationAnimationOptions))
                            }
                            strongSelf.chatListDisplayNode.mainContainerNode.currentItemNode.clearHighlightAnimated(true)
        ...
        self.recentChatsContainerNode.peerAction = { [weak self] peer in
            guard let self else { return }
        ...
        self.recentChatsContainerNode.peerContextAction = { [weak self] peer, node, gesture, point in
            guard let strongSelf = self else {
                gesture?.cancel()
                return
            }
            ...
            if case let .channel(channel) = peer, channel.flags.contains(.isForum) {
                ...
                if peer.id.namespace == Namespaces.Peer.SecretChat, let node = node.subnodes?.first as? ContextExtractedContentContainingNode {
                    ...
                    let chatController = strongSelf.context.sharedContext.makeChatController(context: strongSelf.context, chatLocation: .peer(id: peer.id), subject: nil, botStart: nil, mode: .standard(.previewing), params: nil)
                    chatController.canReadHistory.set(false)
                    contextContentSource = .controller(ContextControllerContentSourceImpl(controller: chatController, sourceNode: node, navigationController: strongSelf.navigationController as? NavigationController))
                }
...
private final class ChatListLocationContext {
    ...
    init(
        ...
        showRecentChats: Signal<Bool, NoError>
    ) {
        self.context = context
        self.location = location
        ...
        case .chatList:
            if !hideNetworkActivityStatus {
                self.titleDisposable = combineLatest(queue: .mainQueue(),
                    ...
                    storyPostingHidden,
                    showRecentChats,
                    childModeStateSignal
                ).startStrict(next: { [weak self] networkState, proxy, passcode, stateAndFilterId, isReorderingTabs, peerStatus, presentationData, storyPostingAvailable, storyPostingHidden, showRecentChats, isChildModeActive in
                    guard let self else {
                        return
                    }
    ...
    private func updateChatList(
        ...
        if stateAndFilterId.state.editing {
            ...
            if titleContent.hasProxy || isChildModeActive {
