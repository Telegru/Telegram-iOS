...
enum DAuthorizationProxyStatus {
    case connecting
    case connected
    case available
}
...
final class DAuthorizationProxyButtonNode: HighlightableButtonNode {
    ...
    var status: DAuthorizationProxyStatus = .available {
        didSet {
    ...
    init(theme: PresentationTheme) {
        ...
        let iconCenter: CGFloat = (44.0 - 30.0) / 2
        let iconFrame = CGRect(origin: CGPoint(x: iconCenter, y: iconCenter), size: CGSize(width: 30.0, height: 30.0))
        ...
        self.frame = CGRect(origin: CGPoint(), size: CGSize(width: 44.0, height: 44.0))
    }
